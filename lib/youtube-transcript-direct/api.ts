/**
 * YouTube get_transcript endpoint (youtubei).
...
const GET_TRANSCRIPT_ENDPOINT_REGEX =
...
export function extractYoutubeiTranscriptConfig(
  html: string
): YoutubeTranscriptConfig | null {
  try {
...
export async function fetchTranscriptFromTranscriptEndpoint(
  ...
  try {
    ...
    if (
      typeof config.pageCl === "number" &&
      Number.isFinite(config.pageCl)
    ) {
    ...
    return extractTranscriptFromTranscriptEndpoint(data);
  } catch {
    return null;
  }
}
...
function extractTranscriptFromTranscriptEndpoint(
  data: unknown
  ...
  const listRenderer = getNestedProperty(
    segmentListNode,
